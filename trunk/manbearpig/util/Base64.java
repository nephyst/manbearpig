...
public class Base64 {
	...
	static public char[] encodeBoolean(boolean[] data) {
	...
	static public boolean[] decodeBoolean(char[] data) {
		byte[] in = decode(data);
		...
		for (int i = 0; i < in.length; i += 1) {
			...
			for (int j = 0; j < 8; j++) {
				...
				boolean value = (mask & current) != 0;
				out[(i * 8) + j] = (boolean)value;
			}
		}
		return out;
	}
	...
	/*
	 * static public boolean[] decode(byte[] data) { boolean[] out = new
	 * boolean[data.length * 8]; for (int i = 0; i < data.length; i++) { byte
	 * shift = (byte)(i%8); byte value = (byte)(data[i] ? 1 : 0); byte mask =
	 * (byte)(value << shift); out[i/8] |= mask; } }/*
	 * 
	 * /** returns an array of base64-encoded characters to represent the passed
	 * data array.
	 * 
	 * @param data the array of bytes to encode @return base64-coded character
	 * array.
	 */
